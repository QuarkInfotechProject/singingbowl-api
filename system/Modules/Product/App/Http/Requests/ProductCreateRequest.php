...
class ProductCreateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'url' => 'required|string|min:2|max:255|unique:products,slug|regex:/^\S*$/',
	    ...
	    'weight' => 'required|numeric|min:0',
            ...
            'files.additionalImage.*' => 'nullable|integer|exists:files,id',
            'files.descriptionVideo' => 'nullable|integer|exists:files,id',

            'options' => $this->getOptionRules(),
            'options.*.name' => ['required', 'string', 'min:2', 'max:30', new UniqueOptionName],
            'options.*.hasImage' => 'required|boolean',
            'options.*.values' => 'required|array',
            'options.*.values.*.optionName' => ['required', 'string', 'min:2', 'max:30', new UniqueOptionValue],
            
            'options.*.values.*.files' => 'nullable|array',
            'options.*.values.*.files.baseImage' => 'integer|exists:files,id',
            'options.*.values.*.files.additionalImage' => 'array',
            'options.*.values.*.files.additionalImage.*' => 'nullable|integer|exists:files,id',

            'variants' => $this->getVariationRules(),
            'variants.*.name' => 'required|string|min:2|max:255',
            'variants.*.sku' => ['required', 'string', 'min:2', 'max:255', new UniqueSkuRule, new WithoutSpacesRule],
            ...
            'variants.*.specialPriceStart' => 'nullable|date|after_or_equal:today',
            'variants.*.specialPriceEnd' => 'nullable|date|required_with:variants.*.specialPriceStart|after:variants.*.specialPriceStart',
            'variants.*.quantity' => 'required|integer|min:1|regex:/^[1-9][0-9]*$/',
            ...
            'activeOfferId.*' => 'nullable|integer',
    ...
    private function getSkuRules() {
        return request()->hasVariant ? 'nullable|string|min:2|max:50|unique:products,sku|regex:/^\S*$/' : 'required|string|min:2|max:50|unique:products,sku|regex:/^\S*$/';
    }

    private function getQuantityRules() {
        return request()->hasVariant ? 'nullable|integer|min:1|regex:/^[1-9][0-9]*$/' : 'required|integer|min:1|regex:/^[1-9][0-9]*$/';
    }
    ...
    private function getOptionValuesFiles() {
        return request()->hasVariant ? 'nullable|array' : 'nullable|array';
    }
    private function getOptionValuesBaseImage() {
        return request()->hasVariant ? 'nullable|integer|exists:files,id' : 'nullable|integer|exists:files,id';
    }
    ...
    public function messages()
    {
        return [
            ...
            'url.unique' => 'This url is already in use. Please choose another one.',
            'url.regex' => 'The url should not contain any white spaces.',
            'brandId.exists' => 'The selected brand does not exist.',
            'brandId.integer' => 'The brandId must be an integer.',
            ...
            'sku.unique' => 'This SKU is already in use. Please choose another one.',
            'sku.regex' => 'This SKU should not contain any white spaces.',
            ...
            'quantity.regex' => 'The quantity must be a positive integer without leading zeros.',
	    ...
	    'weight.required' => 'Please specify the weight of the product.',
            ...
            'files.baseImage.required' => 'Please provide a base image for your product.',
            ...
            'variants.*.sku.unique' => 'This SKU is already in use for another variant. Please choose another one.',
            ...
            'variants.*.quantity.regex' => 'The quantity must be a positive integer without leading zeros.',
            ...
            'activeOfferId.*.integer' => 'The feature id must be a whole number.',
